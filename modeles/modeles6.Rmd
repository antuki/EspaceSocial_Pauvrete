...
bdd_CFA_rmnasup  <- bdd_logit %>% 
  ...
  dplyr::select(s_sentpauvrisque, s_infminidecla,
m_quantilenivie_inv, m_locatif_inv, m_financier_inv,
i_log, i_rsa, i_chom, i_handi, i_bourse, i_hlm, cov_sexe_femme, cov_statut_occup0_proprio, cov_diplome_sans, cov_diplome_bacplus2, cov_diplome_bacplus3, cov_prof_statut_act_chom, cov_prof_statut_act_ouv, cov_prof_statut_act_commer, cov_prof_statut_act_employ, cov_prof_statut_act_autrinac, cov_prof_statut_act_foyer, cov_prof_statut_act_cadre, cov_prof_statut_act_retr, cov_prof_statut_act_agri, cov_prof_statut_act_commer, cov_age_tranche_1829, cov_age_tranche_4049, cov_age_tranche_5059, cov_age_tranche_6069, cov_age_tranche_70, cov_vie_fam_mono, cov_vie_fam_seul, cov_vie_fam_coupenf, cov_vie_fam_autre, cov_vie_fam_enf)
...
i =~ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm 
'
...
# Summary
summary(fit2, standardized = TRUE, fit.measures = TRUE, ci=TRUE)
...
Juste pour l'expérience on teste ces modifications pour voir ! 

```{r, echo=FALSE}
pauv_model2_mi <- '
s =~ s_sentpauvrisque + s_infminidecla
m =~ m_quantilenivie_inv + m_locatif_inv + m_financier_inv + s_sentpauvrisque + s_infminidecla
i =~ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm + s_sentpauvrisque + s_infminidecla
...
### Modèle avec 3 dimensions de la pauvreté (i,m,s) OBLIQUES + AVEC hiérarchie

Il y a un problème avec le modèle, avec des NA pour les standard-error. Comme si le modèle n'était pas bien identifié... Cela semble être dû à l'intégration des corrélations entre s,m et i.
...
summary(fit_hier2, standardized = TRUE, fit.measures = TRUE)

```


### Modèle avec 3 dimensions de la pauvreté (i,m,s) ORTHOGONALES + AVEC hiérarchie

Du coup on enlève les corrélations entre i,m et s pour voir ce que ça donne... Le modèle est pas mal. Je pense que le fait d'intégrer une hiérarchie intègre de fait une corrélation entre les dimensions s,m et i et que si on les rajoute en plus le modèle n'a plus de sens. 
...
fit_hier1 <- lavaan::cfa(pauv_model_hier1, data = bdd_CFA_rmnasup,
ordered = TRUE, orthogonal=TRUE)

semPaths(fit_hier1, what = "est", edge.label.cex = 0.7,
edge.color = 1, esize = 1, sizeMan = 4.5, asize = 2.5,
intercepts = FALSE, rotation = 4, thresholdColor = "red",
mar = c(1, 5, 1.5, 5), fade = FALSE, nCharNodes = 4)

summary(fit_hier1, standardized = TRUE, fit.measures = TRUE)
```

fit_hier1 semble être équivalent à fit2 !

```{r, echo=FALSE}
 semTools::compareFit(fit_hier1, fit2)@fit %>% 
   dplyr::select(chisq, df, pvalue, cfi, tli, rmsea, srmr) 
```

Il y a juste une variance négative (Heywood case) que nous corrigeons en remplaçant la variance légèrement négative de m par une variance légèrement positive (la variance nulle fait aussi bugger le modèle). Le modèle n'est que légèrement moins bien mais permet d'estimer les scores des facteurs latents (avec la fonction predict)

```{r, echo=FALSE}
pauv_model_hier1_corr <- '
s =~ s_sentpauvrisque + s_infminidecla
m =~ m_quantilenivie_inv + m_locatif_inv + m_financier_inv
i =~ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm 
pauvrete =~ s + m + i
...
fit_hier1_corr <- lavaan::cfa(pauv_model_hier1_corr, data = bdd_CFA_rmnasup,
ordered = TRUE, orthogonal=TRUE)

semPaths(fit_hier1_corr, what = "est", edge.label.cex = 0.7,
edge.color = 1, esize = 1, sizeMan = 4.5, asize = 2.5,
intercepts = FALSE, rotation = 4, thresholdColor = "red",
mar = c(1, 5, 1.5, 5), fade = FALSE, nCharNodes = 4)
...
semTools::compareFit(fit_hier1, fit_hier1_corr)@fit %>% 
   dplyr::select(chisq, df, pvalue, cfi, tli, rmsea, srmr) 
```

```{r, echo=FALSE, eval=FALSE}
predictions <- predict(fit_hier1_corr)
...
gg_m_s <- ggplot(df_troisgraphes, aes(x=m, y=s)) +
  ...
  scale_colour_gradientn(name= "Score de\npauvreté",colours = brewer.pal(11,"RdYlGn"))+
  ...
  ylab("Score de pauvreté subjective") + 
  theme_minimal()
gg_m_s
...
gg_s_i <- ggplot(df_troisgraphes, aes(x=s, y=i)) +
  stat_ellipse(aes(fill=factor(typo)),type = "t", color="black", cex=0.5, geom="polygon", alpha=0.6) +
  scale_fill_manual(name="Classe", values=palette, labels=noms_classes) +
  geom_point(aes(color=pauvrete),size=2) +
  scale_colour_gradientn(name= "Score de\npauvreté",colours = brewer.pal(11,"RdYlGn"))+
  ...
  stat_ellipse(aes(color=factor(typo)),type = "t",  cex=1, geom="polygon",fill=NA) +
  scale_color_manual(name="Classe", values=palette, labels=noms_classes, guide="none") +
  xlab("Score de pauvreté subjective") + 
  ylab("Score de pauvreté institionnelle") +
  theme_minimal()
...
m ~~ 0.01*m
# Controles globaux
pauvrete ~ cov_sexe_femme  + cov_diplome_sans + cov_diplome_bacplus2 + cov_diplome_bacplus3 + cov_prof_statut_act_chom + cov_prof_statut_act_ouv + cov_prof_statut_act_commer + cov_prof_statut_act_employ + cov_prof_statut_act_autrinac + cov_prof_statut_act_foyer + cov_prof_statut_act_cadre + cov_prof_statut_act_retr + cov_prof_statut_act_agri + cov_prof_statut_act_commer + cov_age_tranche_1829 + cov_age_tranche_4049 + cov_age_tranche_5059 + cov_age_tranche_6069 + cov_age_tranche_70 + cov_vie_fam_mono + cov_vie_fam_seul + cov_vie_fam_coupenf + cov_vie_fam_autre + cov_vie_fam_enf
'
...
print("fit_hier1_corr : ")
fitMeasures(fit_hier1_corr)[c("chisq","df","pvalue","cfi","tli","rmsea", "srmr")]

```

Le modèle avec contrôles devient moins bien que le précédent.

Voici le détail de l'effet des différents contrôles sur la pauvreté : 
...
kable(tab_covariates_global_cfa)

```


#### Ajout des contrôles sur les indicateurs de pauvretés i,m et s uniquement
...
i ~ cov_sexe_femme  + cov_diplome_sans + cov_diplome_bacplus2 + cov_diplome_bacplus3 + cov_prof_statut_act_chom + cov_prof_statut_act_ouv + cov_prof_statut_act_commer + cov_prof_statut_act_employ + cov_prof_statut_act_autrinac + cov_prof_statut_act_foyer + cov_prof_statut_act_cadre + cov_prof_statut_act_retr + cov_prof_statut_act_agri + cov_prof_statut_act_commer + cov_age_tranche_1829 + cov_age_tranche_4049 + cov_age_tranche_5059 + cov_age_tranche_6069 + cov_age_tranche_70 + cov_vie_fam_mono + cov_vie_fam_seul + cov_vie_fam_coupenf + cov_vie_fam_autre + cov_vie_fam_enf
m ~ cov_sexe_femme  + cov_diplome_sans + cov_diplome_bacplus2 + cov_diplome_bacplus3 + cov_prof_statut_act_chom + cov_prof_statut_act_ouv + cov_prof_statut_act_commer + cov_prof_statut_act_employ + cov_prof_statut_act_autrinac + cov_prof_statut_act_foyer + cov_prof_statut_act_cadre + cov_prof_statut_act_retr + cov_prof_statut_act_agri + cov_prof_statut_act_commer + cov_age_tranche_1829 + cov_age_tranche_4049 + cov_age_tranche_5059 + cov_age_tranche_6069 + cov_age_tranche_70 + cov_vie_fam_mono + cov_vie_fam_seul + cov_vie_fam_coupenf + cov_vie_fam_autre + cov_vie_fam_enf
s ~ cov_sexe_femme  + cov_diplome_sans + cov_diplome_bacplus2 + cov_diplome_bacplus3 + cov_prof_statut_act_chom + cov_prof_statut_act_ouv + cov_prof_statut_act_commer + cov_prof_statut_act_employ + cov_prof_statut_act_autrinac + cov_prof_statut_act_foyer + cov_prof_statut_act_cadre + cov_prof_statut_act_retr + cov_prof_statut_act_agri + cov_prof_statut_act_commer + cov_age_tranche_1829 + cov_age_tranche_4049 + cov_age_tranche_5059 + cov_age_tranche_6069 + cov_age_tranche_70 + cov_vie_fam_mono + cov_vie_fam_seul + cov_vie_fam_coupenf + cov_vie_fam_autre + cov_vie_fam_enf
'

fit_mimic_inter <- lavaan::cfa(pauv_model_mimic_inter, data = bdd_CFA_rmnasup,ordered=TRUE)
...
Le modèle avec contrôles à un niveau intermédiaire est meilleur que le modèle avec contrôles globaux selon les indicateurs de performance. Il y a aussi moins de degrés de liberté. 
...
kable(tab_covariates_inter_cfa[,c("variable","m","i","s")])
```

En général pour tous les contrôles : effet sur s < effet sur i < effet sur m

**Sexe**

Effet non significatif dans la dimension institutionnelle mais significatif et positif dans dimension monétaire (le montant des salaires a un rôle mais pas le fait de toucher une prestation)

**Diplôme**

RAS, effet assez constant

**Vie professionnelle**

- Chômeur : tout est très élevé mais c'est eux qui ont le s le plus élevé de toutes les variables sociodémo. Fort ressenti subjectif
- Ouvrier : subjectif très fort, presque au même niveau que m et i. Idem pour employé dans une moindre mesure. 

**Âge (ref : 30-39)**

- 18-29 : effet positif pour m et i et négatif pour s (les jeunes ne se sentent pas pauvres). 
- 70 : effet très négatif de i (touchent peu de prestations sociales)

**Structure familiale (ref : couple sans enfant)**

- mono : effet bien moins fort du subjectif (on l'avait vu pour la variable s_infminidecla surtout).
- vit seul : c'est l'inverse subjectif presque aussi fort que les deux autres. 
- couple avec enfants : subjectif quasiment nul par rapport à couple sans enfant.  

#### Ajout des contrôles sur les indicateurs de pauvretés i,m et s ET à un niveau global

```{r, echo=FALSE}

pauv_model_mimic_inter_global <- '
s =~ s_sentpauvrisque + s_infminidecla
m =~ m_quantilenivie_inv + m_locatif_inv + m_financier_inv
i =~ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm 
pauvrete =~ s + m + i
...
fit_mimic_inter_global <- lavaan::cfa(pauv_model_mimic_inter_global, data = bdd_CFA_rmnasup,ordered=TRUE)


semPaths(fit_mimic_inter_global, what = "est", edge.label.cex = 0.7,
edge.color = 1, esize = 1, sizeMan = 4.5, asize = 2.5,
intercepts = FALSE, rotation = 4, thresholdColor = "red",
mar = c(1, 5, 1.5, 5), fade = FALSE, nCharNodes = 4)

summary(fit_mimic_inter_global, standardized = TRUE, fit.measures = TRUE)

print("fit_mimic_inter : ")
fitMeasures(fit_mimic_inter)[c("chisq","df","pvalue","cfi","tli","rmsea", "srmr")]

print("fit_mimic_inter_global : ")
fitMeasures(fit_mimic_inter_global)[c("chisq","df","pvalue","cfi","tli","rmsea", "srmr")]
```

Le modèle est légèrement moins bien que précédemment.

Par ailleurs les intervalles de confiance ne sont plus estimés (The information matrix could not be inverted. It may be a symptom that the model is not identified). Comme si cela posait un problème méthodologique d'ajouter les mêmes contrôles aux 2 niveaux. De la même manière que cela posait problème d'ajouter des corrélations entre les 3 dimensions de la pauvreté tout à l'heure. 
