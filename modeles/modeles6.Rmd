...
correlation_ggplot <- function(mat,ordre=row.names(mat),titre_legende="Corrélation", seuilcroix=0.5){
  ...
  cbind(
    ...
    mutate(highval = ifelse(abs(value)>seuilcroix,"1","0")) %>% 
...
m_quantilenivie_inv,m_locatif_inv, m_financier_inv,
i_log, i_rsa, i_chom, i_handi, i_bourse, i_hlm) ~ 1, bdd_poLCA_rmnasup,nrep=1,nclass=nbclasses, graphs=FALSE, na.rm=TRUE, verbose = TRUE,maxiter = 5000)
...
A1(bleu foncé) : Très riches 
A2(bleu moyen) : Riches 
B1 (violet foncé) : Q1 Q3 / **pas infminidecla** / pauvetrisque /  **insti**
B2(violet clair): Q1 Q3 / **infminidecla enorm** / pauvetrisque / **pas insti**
C2(rose clair): Q1/ infminidecla /    **sent pauv et risque** / **peu insti**
C1(rose foncé) : Q2 / infminidecla / **sent pauv** / **inst**
...
```{r, echo=FALSE}
pauv_model0 <- 'pauvrete =~ s_sentpauvrisque + s_infminidecla + m_quantilenivie_inv + m_locatif_inv + m_financier_inv+ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm'


fit0 <- lavaan::cfa(pauv_model0, data = bdd_poLCA_rmnasup,
ordered = TRUE)
...
fit1 <- lavaan::cfa(pauv_model1, data = bdd_poLCA_rmnasup,
ordered = TRUE)
...
### Modèle avec 2 dimensions de la pauvreté (i,m+s) comme dans EFA

```{r, echo=FALSE}
pauv_model2 <- '
m_s =~ m_quantilenivie_inv + m_locatif_inv + m_financier_inv +s_sentpauvrisque + s_infminidecla
i =~ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm 
'

fit2 <- lavaan::cfa(pauv_model2, data = bdd_poLCA_rmnasup,
ordered = TRUE)
...
fit_hier <- lavaan::cfa(pauv_model_hier, data = bdd_poLCA_rmnasup,
ordered = TRUE)
...
pauvrete =~ m_s + i
'
fit_hier2 <- lavaan::cfa(pauv_model_hier2, data = bdd_poLCA_rmnasup,
ordered = TRUE)
...
fit_mimic <- lavaan::cfa(pauv_model_mimic, data = bdd_poLCA,
                         ordered=TRUE)
...
gg_loadings <- ggplot(data.frame(pauvFA2$loadings[,1:2]) %>%
 ...
 geom_abline(intercept = 0, slope = 1, color="black", linetype="solid") +
  ...
  ggrepel::geom_label_repel(data = data.frame(x=c(0.0719,0.673),
           ...
           nudge_x=c(0.3,0), nudge_y=c(0.3,0.3), seed=1) + 
...
gg_latentclass <- ggplot(
  ...
  ggrepel::geom_label_repel(data = data.frame(x=c(-0.761,-0.372,0.0629,0.198, 0.479,0.850),
   ...
   ggrepel::geom_text_repel(data = data.frame(x=c(-0.761,-0.372,0.0629,0.198, 0.479,0.850),
           y=c(0.631,-0.0875,-0.271,0.153,-0.189,0.347)),
            aes(x=x,y=y),
  ...
  geom_segment(aes(x = 0, y = 2, xend = 0, yend = 3), cex=1.5,arrow = arrow(length = unit(0.5, "cm"))) + #1.5
    ggrepel::geom_text_repel(data = data.frame(x=c(0),
           y=c(2)), size = 3, fontface="bold",aes(x=x,y=y),
