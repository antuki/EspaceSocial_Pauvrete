...
bdd_fa <- bdd_logit %>% 
  mutate(s_sentpauv = subj_pauvrete + 0,
         ...
         s_infminidecla = as.numeric(factor(subj_inf_mini_decla,labels=c(1,0)))-1,
...
m_quantilenivie,m_locatif, m_financier,
i_log, i_rsa, i_chom, i_handi, i_bourse, i_hlm) ~ 1, bdd_poLCA_rmnasup,nrep=1,nclass=7, graphs=FALSE, na.rm=TRUE, verbose = TRUE)

# probs.start.new <- poLCA.reorder(result$probs.start,c(4,5,2,1,3))
# 
# result = poLCA(cbind(s_sentpauvrisque, s_infminidecla,
# m_quantilenivie,
# i_log, i_rsa, i_chom) ~ 1, bdd_poLCA_rmnasup,nrep=1,nclass=6, graphs=FALSE, na.rm=TRUE, probs.start=probs.start.new)
...
# s.class(res.mca$ind$coord[,c(1,2)], factor(result$predclass, levels=c(4,5,2,1,3), labels=c(1,2,3,5,6)), col = brewer.pal(7, "Set1")[c(1,2,3,5,6)], sub = "Axes 1 et 2")

s.class(res.mca$ind$coord[,c(1,2)], factor(result$predclass,labels=c(1,2,3,4,5,6,7)), col = brewer.pal(7, "Set1")[c(1,2,3,4,5,6,7)], sub = "Axes 1 et 2")

for(i in 1:5){
poLCA.makeplot.poly.oneclass(result,i, noms_classes = c(1,2,3,4,5))
}
...
m =~ m_nivie1 + m_nivie2 + m_nivie4 + m_nivie5 
i =~ i_log + i_rsa + i_chom + i_handi 
'
