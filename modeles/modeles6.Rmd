...
correlation_ggplot(mat=cor_spearman, ordre = rev(c("m_quantilenivie","m_locatif","m_financier","s_infminidecla","s_sentpauvrisque","i_hlm","i_handi","i_chom","i_rsa","i_log","i_bourse")),
...
correlation_ggplot(mat=bdd_poLCA_poly$rho, ordre = rev(c("m_quantilenivie","m_locatif","m_financier","s_infminidecla","s_sentpauvrisque","i_hlm","i_handi","i_chom","i_rsa","i_log","i_bourse")), titre_legende = "Corrélation Poly")
...
probs.start.new <- poLCA.reorder(result$probs.start,1:6)
...
noms_classes <- c("A","B","C1","C2","D","E")
ordre_classes <- c(5,2,1,4,6,3)
ordre_plots <- c(3,2,6,4,1,5)
...
s.class(res.mca$ind$coord[,c(1,2)], factor(result$predclass,labels=noms_classes[ordre_classes]), col = c(brewer.pal(5, "Set1")[1:3],"#193818",brewer.pal(5, "Set1")[4:5])[ordre_classes], sub = "Axes 1 et 2") #"#2d692b",

for(i in ordre_plots){
poLCA.makeplot.poly.oneclass(result,i, noms_classes = noms_classes[ordre_classes])
}
...
```{r, echo=FALSE}
pauv_model0 <- 'pauvrete =~ s_sentpauvrisque + s_infminidecla + m_quantilenivie + m_locatif + m_financier+ i_log + i_rsa +  i_chom + i_handi + i_bourse + i_hlm'
...
m =~ (-1)*m_quantilenivie + m_locatif + m_financier 
