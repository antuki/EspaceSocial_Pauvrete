...
## Données actualisées (2016-2019) [**NEW**]
...
```{r, echo=FALSE}
#Chargement de la base de données
bdd2015 <- readRDS("../data/2019/barometre2000_2019_diff.rds") %>% 
  filter(annee%in%2015:2019)

revenus_a_imputer <- bdd2015 %>%
  mutate(sdrevcl = ifelse(sdrevcl==999999999,NA,sdrevcl)) %>%
  group_by(sdrevtr) %>% 
  summarise(sdrevcl = mean(sdrevcl,na.rm=TRUE))
...
bdd2015 <- bdd2015 %>% 
   mutate(seuil_pauvrete = do.call(c,lapply(annee,function(annee_selec){
0.6*median(bdd2015[bdd2015$annee==annee_selec,"sdrevcl_imput_moy"]/bdd2015[bdd2015$annee==annee_selec,"sduc"],
           na.rm=TRUE)
}
))) %>% 
   mutate(subj_inf_mini_decla = sdrevcl_imput_moy - ifelse(pe16==999999999,NA,pe16) <0) %>% 
...
bdd2015 <- bdd2015 %>% 
  mutate(quantile_nivie = factor(
    ...
    labels=rev(paste0("Quintile ",c(1,2,3,4,5))))
  )
```
...
fig_intro <- bdd2015 %>%
  ...
  ungroup() %>% 
  mutate(#quantile_nivie=factor(quantile_nivie,labels=names(attr(bdd2015$quantile_nivie,"labels"))),
         pe3=factor(pe3,labels=
           c(names(attr(bdd2015$pe3,"labels"))[1:2],"Je me considère déjà\ncomme pauvre",names(attr(bdd2015$pe3,"labels"))[4])), 
  ...
  theme_minimal() +
  scale_fill_manual(name= paste(strwrap(attr(bdd2015$pe3,"label"),30),collapse="\n"),
...
fig_chap1_compa20152019 <- ggplot(data=data_g1_new %>% tidyr::pivot_longer(!annee),
       aes(x=name, y=value, fill=annee)) +
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=value), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+
...
bdd_logit <- bdd %>% 
  ...
  mutate(annee_fac = factor(annee,labels=paste0("-",2016:2019))) %>%  #NEW
  mutate(pauvrete_mon_rel= factor(
    ifelse(sdrevcl_imput_moy/sduc<seuil_pauvrete,3,
                                  ifelse(sdrevcl_imput_moy/sduc>=1.2*seuil_pauvrete,1,
                                         ifelse(sdrevcl_imput_moy/sduc<1.2*seuil_pauvrete &
...
tab_compa <- tab_compa_old %>% bind_cols(tab_compa_new %>% select(odds_ratio)) %>% 
  setNames(c("Modèle logit"," Variable dépendante : se déclarer pauvre", "Odds ratio - Années 2015 à 2017","Odds ratio - Années 2016 à 2019"))
