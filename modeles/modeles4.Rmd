...
```{r, echo=FALSE}
# run_regression(y="subj_pauvrete",
#                #x= c("est_rsa","presta_rsa","conn_rsa"),
#                x= c("est_rsa","presta_rsa","conn_rsa_fam",
#                     "conn_rsa_horsfam"),
#                kable=TRUE)

tab_rsa <- run_regression(y="subj_pauvrete", 
                       #x= c("est_rsa","presta_rsa","conn_rsa"),
                       x= c("est_rsa","presta_rsa","conn_rsa_fam",
                    "conn_rsa_horsfam"),
               kable=FALSE)  %>%
  ...
  slice(-1) %>%
  mutate(modalite = c("N'est pas une personne au RSA", "Est une personne au RSA", "N'a personne dans son ménage au RSA", "A quelqu'un dans son ménage au RSA", "Ne connaît personne dans sa famille au RSA", "Connaît quelqu'un dans sa famille au RSA", "Ne connaît personne hors famille au RSA", "Connaît quelqu'un hors famille au RSA")
         ) %>% 
  setNames(c("Modèle logit. Variable dépendante : se déclarer pauvre", "Odds ratio")) 

saveRDS(tab_rsa,"../rapport/figures/tab_rsa.RDS")
...
```{r, echo=FALSE}
#  run_regression(x= c("quantile_nivie", "revenus_salaires","revenus_independant","revenus_retraite","revenus_financiers","revenus_locatifs"), y="subj_pauvrete",kable=TRUE)

tab_mon <- run_regression(x= c("quantile_nivie", "revenus_salaires","revenus_independant","revenus_retraite","revenus_financiers","revenus_locatifs"), y="subj_pauvrete",kable=FALSE) %>% 
  mutate(odds_ratio = ifelse(
    !is.na(valeur),paste0(gsub("\\.",",",odds),pvaleur),"Réf."
  )) %>% 
...
```{r, echo=FALSE}
# run_regression(x= c("presta_rsa","presta_chomage","presta_apl","presta_handi","presta_etudes", "presta_hlm"), y="subj_pauvrete",kable=TRUE)


tab_inst <- run_regression(x= c("presta_rsa","presta_chomage","presta_apl","presta_handi","presta_etudes", "presta_hlm"), y="subj_pauvrete",kable=FALSE) %>% 
  mutate(odds_ratio = ifelse(
    !is.na(valeur),paste0(gsub("\\.",",",odds),pvaleur),"Réf."
  )) %>% 
  select(modalite,odds_ratio) %>% 
  slice(-1) %>% 
  #arrange(match(modalite, c(modalites[2],modalite[-2]))) %>% 
 setNames(c("Modèle logit. Variable dépendante : se déclarer pauvre", "Odds ratio")) 
...
# "statut_occup0","prof_statut_act","est_emploiprecaire", "diplome", "age_tranche","vie_fam", "sexe", "conn_rsa", "annee_fac"),
...
"statut_occup0","prof_statut_act","est_emploiprecaire", "diplome", "age_tranche","vie_fam", "sexe", "conn_rsa", "annee_fac"),
  ...
  mutate(modalite_y = c("Niveau de vie",rep("",4),"Logement",rep("",1), "Situation professionnelle",rep("",9),"Précarité de l'emploi","(subjective)","Niveau de diplôme le plus élevé",rep("",3), "Classe d'âge",rep("",5),"Situation familiale",rep("",5),"Sexe",rep("",1),"Entourage au RSA",rep("",1),"Année",rep("",4))) %>% setNames(c("Modèle logit", "Variable dépendante : appartenir à la classe C.2 plutôt que C.1", "Odds ratio")) 
...
saveRDS(gg_taux_viefam,"../rapport/figures/gg_taux_viefam.RDS")
...
```{r, echo=FALSE, fig.height=6, fig.width=7}
gg_taux_age <- function(df, legend_only=FALSE,
                           legend.position="none", sous_champ="Titre"){
  df <- df %>% 
    filter(!vie_fam%in%c("-Autre situation familiale")) %>%
    mutate(vie_fam = gsub("-","",vie_fam)) %>%         
  ...
  p <- ggplot(df, aes(x=nivie, y=ratio, group=age_tranche, color=age_tranche)) +
    #geom_line() +
    geom_smooth(method = "loess", size=1)+ #lm, glm, loess, NULL, gam
    scale_color_brewer(palette="Set2")+
    theme_minimal()+
    ...
    ggtitle(sous_champ) +
    guides(linetype = guide_legend(title="Champ",order = 1),
           color = guide_legend(title="Tranche\nage", order = 2)
    )+
    theme(title=element_text(size=8, face="bold"))
...
leg <- gg_taux_age(df=bdd_logit %>% mutate(indicatrice = ifelse(presta_rsa=="-RSA",1,0)), legend_only = TRUE)
p1 <- gg_taux_age(df=bdd_logit %>% mutate(indicatrice = ifelse(presta_rsa=="-RSA",1,0)), sous_champ="Français qui bénéficient du RSA")
p2 <- gg_taux_age(df=bdd_logit %>% mutate(indicatrice = ifelse(presta_apl=="-APL",1,0)), sous_champ="Français qui bénéficient d'APL")
p3 <- gg_taux_age(df=bdd_logit %>% mutate(indicatrice = ifelse(subj_pauvrete==1,1,0)), sous_champ="Français qui se sentent pauvres")
p4 <- gg_taux_age(df=bdd_logit %>% mutate(indicatrice = ifelse(subj_inf_mini_decla==1,1,0)), sous_champ="Français se sentant en difficulté fin.") #"-Rev. inf. au minimum décla."
library(cowplot)

gg_taux_age <- plot_grid(NULL,NULL,p1,p2,p3,p4,nrow=3, rel_heights = c(1,3,3))+
  draw_plot(leg,x=0,y=0.45)
