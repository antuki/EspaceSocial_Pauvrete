...
creer_x <- function(typo){
  ...
  x$probs <- lapply(colnames(x$y),function(var){
    ...
    t <- t/rowSums(t)
    row.names(t) <- paste0("class ", row.names(t),":")
    colnames(t) <- paste0("Pr(", colnames(t),")")
    return(t)  
  })
...
res.mca <- MCA(bdd_acm,
               ...
               graph=FALSE) 
#explor(res.mca)
#saveRDS(res.mca,"../data/res.mca2.RDS")
res <- explor::prepare_results(res.mca)
...
explor::MCA_var_plot(res, xax = 1, yax = 2, var_sup = FALSE, var_sup_choice = ,
    var_lab_min_contrib = 0, col_var = "Variable", symbol_var = "Type", size_var = NULL,
    size_range = c(10, 300), labels_size = 10, point_size = 56, transitions = TRUE,
    labels_positions = NULL, labels_prepend_var = FALSE, xlim = c(-2, 2),
    ylim = c(-1, 2))

explor::MCA_var_plot(res, xax = 1, yax = 2, var_sup = TRUE, var_sup_choice = c("statut_occup",
    "diplome", "conn_rsa", "annee_fac", "prof_statut_act", "age_tranche", "vie_fam"),
    var_lab_min_contrib = 0, col_var = "Type", symbol_var = "Variable", size_var = NULL,
    size_range = c(10, 300), labels_size = 10, point_size = 56, transitions = TRUE,
    labels_positions = NULL, labels_prepend_var = FALSE, xlim = c(-1.16, 1.32),
    ylim = c(-0.974, 1.5))
