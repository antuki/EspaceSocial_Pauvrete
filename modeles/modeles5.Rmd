...
creer_x <- function(typo){
  ...
  x$y <- bdd_poLCA_rmnasup %>% dplyr::select(-ident)
...
typo <- factor(factor(cutree(arbre,k=nbclasses),labels=c(5,3,4,2,1)),levels=1:nbclasses)
...
gg_cah <- ggplot(data.frame(res.mca$ind$coord[,c(1,2)] %>% cbind(typo)),
  ...
  ggrepel::geom_label_repel(data = data.frame(x=c(-0.792,-0.471,-0.248,0.232,0.848),
           ...
           aes(x=x,y=y), label = paste0('Classe ',1:5),
  ...
  ggrepel::geom_text_repel(data = data.frame(x=c(-0.792,-0.471,-0.248,0.232,0.848),
           y=c(0.719,0.180,-0.271,-0.205,0.342)),
           aes(x=x,y=y),
           label = c("Richesse très importante", "Richesse importante", "Classes moyennes", "Pauvreté (surtout monétaire)", "Pauvreté (surtout institutionnelle)"), 
...
#geom_enterotype(aes(color=factor(typo))) #+
  ...
  #                   show.legend=FALSE) +
   xlab("Axe 1 (19,8 %)") +
  ylab("Axe 2 (9,6 %)") + 
  scale_color_manual(values=rev(RColorBrewer::brewer.pal(6, "BrBG")[c(1:2,4:6)])) +
  scale_fill_manual(values=rev(RColorBrewer::brewer.pal(6, "BrBG")[c(1:2,4:6)])) +
...
gg_cah_bar <- do.call("rbind", 
                 lapply(1:length(x$probs), function(i){
                   ...
                   df$typo=noms_classes
                   return(df)
                   })
  )  %>%
    mutate(Var2=as.numeric(Var2)) %>% 
  mutate(Var2=ifelse(!var%in%c("s_sentpauvrisque","m_quantilenivie") & Var2==1,100,Var2)) %>% 
  mutate(Var2=ifelse(!var%in%c("s_sentpauvrisque","m_quantilenivie") & Var2==2,1,Var2)) %>% 
  mutate(Var2=ifelse(!var%in%c("s_sentpauvrisque","m_quantilenivie") & Var2==100,5,Var2)) %>% 
  ...
  mutate(Var2=ifelse(var=="s_sentpauvrisque" & Var2==1,100,ifelse(var=="s_sentpauvrisque" & Var2==3,1,Var2))) %>% 
   ...
   mutate(Var2=factor(Var2)) %>% 
    ...
    levels=c("m_quantilenivie","m_locatif","m_financier","s_sentpauvrisque", "s_infminidecla", "i_rsa", "i_log", "i_hlm","i_chom","i_bourse","i_handi") ,
...
"Ménage bénéficiaire\ndu RSA ?", "Ménage bénéficiaire\nd'APL ?","Ménage locataire\nd'un logement social ?","Ménage bénéficiaire\nd'allocation chômage ?","Ménage bénéficiaire\nde bourse d'étude ?", "Ménage bénéficiaire\nd'une allocation handicap\n/ dépendance ?")
    )) %>% mutate(typo=paste0("Classe ", noms_classes[typo],                        "\n(",gsub("\\.",",",round(100*x$P[typo],1))," %)")) %>% 
  ggplot(aes(x=var, y=100*Freq)) +
   geom_rect(aes(fill = typo),xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf) +
   annotate("rect", xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf, alpha=0.7, fill="white") +
 scale_fill_manual(values=rev(RColorBrewer::brewer.pal(6, "BrBG")[c(1:2,4:6)]),guide="none") +
  ...
  scale_y_continuous(breaks=c(0,20,40,60,80,100), expand = c(0, 20)) +
   ...
   # theme(axis.text.x = element_text(angle = 30, hjust=0), 
    theme(axis.text.x = element_text(angle = 90,vjust = 0.5, hjust=1), 
          strip.text = element_text(aes(colour = typo), face="bold"))
