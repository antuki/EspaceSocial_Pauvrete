...
Nous avons cherché dans un premier temps à représenter grâce à une Analyse des Correspondances Multiples (ACM) (voir encadré 1) l’espace social de la pauvreté issu des données du Baromètre d’opinion de la Drees.
...
if(knitr:::is_latex_output()){
note <- c("Dans la classe 3, 35 \\% des individus appartiennent à des ménages du troisième quintile de niveau de vie, aucun individu n’appartient à un ménage qui reçoit des revenus locatifs, 1 \\% indiquent se sentir pauvre et 12 \\% indiquent risquer le devenir dans les cinq prochaines années.")
} else{
 note <- c("Dans la classe 3, 35 % des individus appartiennent à des ménages du troisième quintile de niveau de vie, aucun individu n’appartient à un ménage qui reçoit des revenus locatifs, 1 % indiquent se sentir pauvre et 12 % indiquent risquer le devenir dans les cinq prochaines années.")  
}
...
```{r cah2, fig.show='hold', fig.cap='Indicateurs des différentes dimensions de la pauvreté pour chacune des 5 classes de la CAH', echo=FALSE, fig.width=7, fig.height=6, out.width="100%",fig.source=source, fig.champ=champ, fig.note = note}
...
tab_modfit %>%
  ...
  kableExtra::column_spec(2, width = "1.7cm") %>% 
  kableExtra::column_spec(3:4, width = "1.5cm") %>% 
  kableExtra::column_spec(5:6, width = "1.1cm") %>% 
...
```{r afe, fig.show='hold', fig.cap='Espace des variables de pauvreté monétaire, institutionnelle et sujective (Analyse en Facteurs Communs)', echo=FALSE, fig.width=7, fig.height=5, out.width="100%",fig.source=source, fig.champ=champ, fig.lecture=lecture}
