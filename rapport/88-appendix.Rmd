...
*Source : inspiré du tutoriel de l'UCLA [A Practical Introduction to Factor Analysis: Exploratory Factor Analysis](https://stats.idre.ucla.edu/spss/seminars/introduction-to-factor-analysis/a-practical-introduction-to-factor-analysis/)*
...
Comme le nombre de variables inconnues (les $m$ facteurs communs et $p$ facteurs uniques) est supérieur au nombre de variables connues ($p$ variables observées), le modèle est indéterminé. Différentes hypothèses supplémentaires sont nécessaires pour obtenir une formule pour la matrice de corrélations $R$ : 
...
Par ailleurs, pour que le modèle soit bien déterminé, on doit avoir $p > \frac{1}{2} [ (2m + 1) + \sqrt{8m + 1} ]$ (@ledermann1937rank) c’est-à-dire au moins 3 variables pour 1 facteur, 5 variables pour 2 facteurs, 6 variables pour 3 facteurs, etc.
...
-	$\Lambda [p, m]$ une matrice des saturations des $p$ variables observées sur les m facteurs et
...
Alors que l'Analyse en Composantes Principales (ACP, @hotelling1933analysis) concerne des variables uniquement continues, l'Analyse Factorielle des Correspondances (@benzecri1982histoire) permet d'analyser le lien entre deux variables discrètes. Concrètement, il s’agit de réaliser une ACP sur les profils lignes et colonnes du tableau de contingence en utilisant une distance du $\chi_2$.
...
A l’inverse, l’AF différencie variances commune et unique afin de chercher à identifier des facteurs latents. En Elle cherche, contrairement aux méthodes d’analyse factorielle, à comprendre la structure des corrélations (termes non diagonaux de la matrice $\Sigma$) entre les variables mesurées dans la base de données et ne se focalisent pas principalement sur la variance (termes diagonaux). Les facteurs construits tiennent ainsi compte de la variance commune plutôt que totale comme dans une analyse factorielle et la corrélation entre deux variables dépend alors de la similarité de leur relation avec les facteurs latents.

Malgré ces différences, les méthodes d’analyse factorielle peuvent dans certains cas se substituer à l’analyse en facteurs communs. De nombreux articles, comme @velicer1990component, indiquent même qu’il y a en pratique très peu de différences entre les deux approches dans la majeure partie des cas^[@velicer1990component vont même plus loin en indiquant que les différences entre méthodes se font surtout sentir quand les modèles s'estiment mal, avec des loadings très faibles et que ces différences sont souvent en défaveur de l'AF.] et que, même mathématiquement, ces méthodes sont voisines. Les méthodes d’analyse factorielle ont aussi comme gros avantage un fonctionnement computationnel simplifié car elles utilisent des méthodes d’estimations algébriques, plus simples que des méthodes types maximum de vraisemblance mobilisées en AF. Les méthodes d’AF se confrontent parfois aux « Heywood cases » (évoqués plus haut), c’est-à-dire des situations où les communautés des variables mesurées sont supérieures ou égales à 1 et faussent la modélisation (une variable ne peut pas résumer plus de 100 % de la variance).

Mais même si ces deux approches produisent parfois des résultats similaires, ce n’est pas le cas dans de nombreux contextes : quand les données contiennent des erreurs de mesure (rendues compte dans les AF mais pas en analyse factorielle), ou des valeurs extrêmes (non supportées en analyse factorielle). Par ailleurs, les modèles en facteurs communs peuvent être testés car supposent certaines hypothèses structurelles sur les données. Le modèle est rejeté si mal estimé par les données, chose qui ne peut pas être faite avec des analyses factorielles. 
