...
title: |
    | Figures
subtitle : "Au-delà du revenu : la place de la dimension subjective dans la structure de la pauvreté en France"
author: |
    | Kim Antunez, Ivaylo Petev
...
tableauA1 <- df_fin %>% 
  kable(booktabs = TRUE,longtable = TRUE) %>% 
   ...
   kableExtra::footnote(c("")) #ajoutée pour faciliter le copier coller html
...
```{r, echo=FALSE}
#N = 13567 / R2 ajusté de McFadden = 25.9
...
for(couple in list(c(2,3),c(24,25),c(25,26),
                   c(26,27),c(36,37),c(40,41),
                   c(46,47)
                   )){
  tab_final21_sentpauv[couple,] <- tab_final21_sentpauv[rev(couple),]
}



tab_final21_sentpauv <- tab_final21_sentpauv %>% 
...
```{r, echo=FALSE}
#N = 13698 / R2 ajusté de McFadden = 28.6
tab_final21_infminidecla <- run_regression(y="subj_inf_mini_decla", 
               x= c("quantile_nivie","revenus_locatifs","revenus_financiers","presta_rsa","presta_apl","presta_chomage","presta_handi",
                     "presta_etudes",
                    "statut_occup", "prof_statut_act","est_emploiprecaire", "diplome", "age_tranche","vie_fam", "sexe", #"conn_rsa",
                    "annee_fac"
), bdd_log=bdd_logit %>% mutate(subj_inf_mini_decla = ifelse(as.numeric(subj_inf_mini_decla)==2,0,1)), kable = FALSE) 

for(couple in list(c(2,3),c(24,25),c(25,26),
                   c(26,27),c(36,37),c(40,41),
                   c(46,47)
                   )){
  tab_final21_infminidecla[couple,] <- tab_final21_infminidecla[rev(couple),]
}

tab_final21_infminidecla <-tab_final21_infminidecla %>% 
  mutate(odds_ratio = ifelse(
    !is.na(valeur),paste0(gsub("\\.",",",odds),pvaleur),"Réf."
  )) %>% 
  dplyr::select(modalite_y,modalite,odds_ratio) %>% 
...
tab_final21 %>% 
kableExtra::footnote(c("Sentiment de pauvreté : N = 13567 et $R^2$ ajusté de McFadden = 25,9 \\\\, \\\\%","Difficultés financières perçues : N = 13698 et $R^2$ ajusté de McFadden = 28,6 \\\\, \\\\%",
...
kableExtra::kable_styling(
  ...
  repeat_header_continued = "\\textit{(suite en page suivante...)}"
    ...
    kableExtra::footnote(c("Modèle 2015-2017 : N = 8460 et $R^2$ ajusté de McFadden = 24,8 \\\\, \\\\%","Modèle 2015-2019 : N = 13590 et $R^2$ ajusté de McFadden = 25,5 \\\\, \\\\%.",
...
```{r, echo=FALSE}
#N = 13548 / R2 ajusté de McFadden = 26.1 \%
...
```{r, echo=FALSE}
#N = 13678 / R2 ajusté de McFadden = 28.6 \%
tab_final21_infminidecla_bis <- run_regression(y="subj_inf_mini_decla", 
               x= c("quantile_nivie","revenus_locatifs","revenus_financiers","presta_rsa","presta_apl","presta_chomage","presta_handi",
                    ...
                    #"conn_rsa",
                    "annee_fac"), 
                    bdd_log=bdd_logit %>% mutate(subj_inf_mini_decla = ifelse(as.numeric(subj_inf_mini_decla)==2,0,1))
, kable = FALSE) %>% 
  mutate(odds_ratio = ifelse(
    !is.na(valeur),paste0(gsub("\\.",",",odds),pvaleur),"Réf."
  )) %>% 
...
tab_final21_bis %>% 
kableExtra::footnote(c("Sentiment de pauvreté : N = 13548 et $R^2$ ajusté de McFadden = 26.1 \\\\, \\\\%","Difficultés financières perçues : N = 13678 et $R^2$ ajusté de McFadden = 28,6 \\\\, \\\\%",
                          "* : significatif au seuil de $5 \\\\, \\\\%$ ; ** : $1 \\\\, \\\\%$ ; *** : $0,1 \\\\, \\\\%$."), escape = FALSE)  #%>% add_footnote_kable()
```
...
0.6*median(bdd2018[bdd2018$annee==annee_selec,"sdrevcl_imput_moy"]/bdd2018[bdd2018$annee==annee_selec,"sduc"],
  ...
  mutate(ins_grave_pb_perso = 
           ...
           labels=c("Pouvoir compter sur quelqu'un en cas de grave problème personnel","Ne pas pouvoir compter sur quelqu'un en cas de grave problème personnel"))
         ) %>%
...
kable(#caption= "Part des personnes concernées par des situations d'insécurité ou infériorité en fonction de leur sentiment de pauvreté (%)",
      booktabs = TRUE, longtable = TRUE)  %>%
     kableExtra::kable_styling(latex_options = c("hold_position", "repeat_header")) %>% 
...
```{r corrplot_data, echo=FALSE, fig.width=7, fig.height=5}
# library(ggnewscale)
...
data_mat <- cbind(
  ...
  data.frame(ggcorrplot::cor_pmat(mat))  %>%
    ...
    mutate(value=ifelse(value>0.10,"Non","Oui")) %>% 
    dplyr::select(value) %>% 
    setNames("signi")
)  %>% 
  mutate(label = round(value,2)) %>% 
...
```{r corrplot_old, echo=FALSE, eval=FALSE, fig.width=7, fig.height=5}
# Ancienne figure 3 avant juillet 2024
gg_corrplot_old <-  ggplot(data_mat) + ## to get the rect filled
  geom_tile(aes(x=mesure1, y=mesure2), color="white", fill="white") +
  geom_point(aes(x=mesure1, y=mesure2, fill=value, stroke= highval),size = 5, shape=21) +
 # geom_point(aes(x=mesure1, y=mesure2, shape = highval),size = 2,color = "black")+
  labs(x = NULL, y = NULL, fill = "Corrélation Spearman") +
...
# Nouvelle figure 3 après juillet 2024
gg_corrplot <-  ggplot(data_mat) +
  geom_tile(aes(x=mesure1, y=mesure2), color="white", fill="white") +
  labs(x = NULL, y = NULL, fill = "Corrélation Spearman\nsignificative à 5 %") +
   geom_point(aes(x=mesure1, y=mesure2, stroke= highval, fill=signi),size = 9, shape=21) +
    geom_label(aes(x=mesure1, y=mesure2, label=label),size = 3, label.size=0, fill=NA) +
  theme_classic() +
     continuous_scale("stroke", "stroke", name= paste0("Corrélation (val.\nabs.) > ",seuilcroix),
                    palette = function(x){scales::rescale(x, c(1, 2))},
                    breaks = c(2, 1), labels=c("Oui","Non")) +
  ...
  scale_x_discrete(expand=c(0.05,0.05)) +
  scale_y_discrete(expand=c(0.05,0.05)) +
  scale_size(range=c(2,5), guide=NULL) +
  geom_rect(data=data.frame(x1=c(0.5,3.5,5.5), x2=c(3.5,5.5,11.5), y1=c(0.5,3.5,5.5), y2=c(3.5,5.5,11.5), t=c('Monétaire','Subjective','Institutionnelle')), mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2, linetype=t), color="black", size=1, fill=NA) +
  scale_linetype_manual(name="Dimension de la pauvreté", breaks=c("Monétaire","Subjective","Institutionnelle"), values=c("dotted","dashed", "solid")) +
 theme(axis.text.x = element_text(angle = 45, hjust=1))  
...
# Statsitiques ajustement modèle tableau 5 demandé par Ivaylo le 25/02/26
tab_modfit_mimic <- data.frame(
  Modele = c("MIMIC 1D (Monétaire)",
             "MIMIC 2D (Monétaire + institutionnel)",
             "MIMIC 3D (Monétaire + institutionnel + subjectif)"),
  chisq = NA,
  df = NA,
  cfi = NA,
  rmsea = NA,
...
# 1D : monétaire seul
f1 <- fitMeasures(fit_mimic_global_new2)
tab_modfit_mimic[1, 2:6] <- c(round(f1[c("chisq","df")]),
                              gsub("\\.", ",", round(f1[c("cfi","rmsea", "srmr")], 3)))

# 2D : monétaire + institutionnel
f2 <- fitMeasures(fit_mimic_global_new1)
tab_modfit_mimic[2, 2:6] <- c(round(f2[c("chisq","df")]),
                              gsub("\\.", ",", round(f2[c("cfi","rmsea", "srmr")], 3)))

# 3D : monétaire + institutionnel + subjectif
get_fit_safe <- function(fit) {
  out <- c(
...
f3 <- get_fit_safe(fit_mimic_global)
names(f3) <- c("chisq","df","cfi","rmsea","srmr")
tab_modfit_mimic[3, 2:6] <- c(round(f3[c("chisq","df")]),
                              gsub("\\.", ",", round(f3[c("cfi","rmsea", "srmr")], 3)))
